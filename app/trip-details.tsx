...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  active:    { label: 'جارية',        color: Colors.primary },
  completed: { label: 'مكتملة',       color: Colors.success },
  cancelled: { label: 'ملغاة',        color: Colors.error },
};

function formatDateTime(iso: string) {
  const d = new Date(iso);
  return {
    date: d.toLocaleDateString('ar-EG', { day: '2-digit', month: 'long', year: 'numeric' }),
...
export default function TripDetailsScreen() {
  ...
  const handleReorder = () => {
    ...
    showAlert(
      'إعادة الطلب',
      `هل تريد إعادة طلب رحلة مع ${trip.drivers.name}؟`,
      [
        { text: 'إلغاء', style: 'cancel' },
        { text: 'نعم، اطلب', onPress: () => router.push(`/driver/${trip.drivers!.id}` as any) },
      ]
    );
  };
  ...
  if (loading) {
    return (
      <View style={[styles.container, styles.center, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.loadingText}>جارٍ تحميل تفاصيل الرحلة...</Text>
      </View>
    );
  }
  ...
  if (!trip) {
    return (
      <View style={[styles.container, styles.center, { paddingTop: insets.top }]}>
        <StatusBar style="light" />
        <LinearGradient colors={[Colors.bgDark, Colors.bgNavy]} style={StyleSheet.absoluteFillObject} />
        <MaterialIcons name="error-outline" size={64} color={Colors.textLight} />
        ...
        <TouchableOpacity style={styles.backBtnCenter} onPress={() => router.back()}>
          <Text style={styles.backBtnCenterText}>العودة</Text>
        </TouchableOpacity>
      </View>
    );
  }

  const { date, time } = formatDateTime(trip.created_at);
  const statusCfg = STATUS_MAP[trip.status] ?? STATUS_MAP['pending'];
  const price = Number(trip.price);
  const baseFare = Math.round(price * 0.6);
  const distanceFare = Math.round(price * 0.3);
  const serviceFee = Math.round(price - baseFare - distanceFare);

  const driver = trip.drivers;
  const driverName = driver?.name ?? 'سائق تك توكي';
  const driverVehicle = driver?.vehicle ?? 'توك توك';
  ...
  const driverVehicleType = driver?.vehicle_type ?? 'توك توك';
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <LinearGradient colors={[Colors.bgDark, Colors.bgNavy]} style={styles.header}>
        ...
        <View style={styles.tripMeta}>
          <Text style={styles.tripDate}>{date} · {time}</Text>
          <Text style={styles.tripId}>#{trip.id.slice(0, 8).toUpperCase()}</Text>
        </View>
      </LinearGradient>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 100 }}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.routeBlock}>
            <View style={styles.routeItem}>
              ...
              <View style={styles.routeInfo}>
                ...
                <Text style={styles.routeText}>{trip.from_location}</Text>
              </View>
            </View>
            ...
            <View style={styles.routeItem}>
              ...
              <View style={styles.routeInfo}>
                ...
                <Text style={styles.routeText}>{trip.to_location}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.statsRow}>
            {[
              ...
              { label: 'التقييم', value: (trip.rating ?? 0) > 0 ? `${trip.rating} ★` : '—', icon: 'star', color: Colors.accent },
            ].map((s, i) => (
          ...
          {trip.status === 'cancelled' && trip.cancellation_reason ? (
            <View style={styles.cancelNote}>
              <MaterialIcons name="info-outline" size={15} color={Colors.error} />
              <Text style={styles.cancelNoteText}>{trip.cancellation_reason}</Text>
            </View>
          ) : null}
        </View>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity
            ...
            onPress={() => driver ? router.push(`/driver/${driver.id}` as any) : null}
            activeOpacity={driver ? 0.85 : 1}
          >
            <MaterialIcons name="chevron-left" size={20} color={driver ? Colors.textLight : 'transparent'} />
            <View style={styles.driverInfo}>
              <Text style={styles.driverName}>{driverName}</Text>
              <Text style={styles.driverVehicle}>{driverVehicle}</Text>
              <View style={styles.ratingRow}>
                {[1,2,3,4,5].map(i => (
                  <MaterialIcons key={i} name="star" size={13} color={i <= driverRating ? Colors.accent : Colors.border} />
                ))}
            ...
            {driverAvatar ? (
              ...
              <View style={[styles.driverAvatar, styles.avatarPlaceholder]}>
                <MaterialIcons name="person" size={26} color={Colors.textLight} />
              </View>
            )}
          </TouchableOpacity>
          ...
          <View style={styles.vehicleDetails}>
            <View style={styles.vehicleDetailItem}>
              <Text style={styles.vehicleDetailVal}>{driverPlate}</Text>
              <Text style={styles.vehicleDetailLabel}>رقم اللوحة</Text>
            </View>
            <View style={styles.vehicleDetailDivider} />
            <View style={styles.vehicleDetailItem}>
              <Text style={styles.vehicleDetailVal}>{driverVehicleType}</Text>
              <Text style={styles.vehicleDetailLabel}>نوع المركبة</Text>
            </View>
            <View style={styles.vehicleDetailDivider} />
            <View style={styles.vehicleDetailItem}>
              <Text style={styles.vehicleDetailVal}>{driverTrips.toLocaleString()}</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.priceRows}>
            {[
              { label: 'الأجرة الأساسية', value: baseFare },
            ...
            <View style={[styles.priceRow, styles.priceTotalRow]}>
              <Text style={styles.priceTotalVal}>{price.toFixed(0)} ج.م</Text>
          ...
          <View style={styles.paymentMethodRow}>
            <View style={styles.paymentBadge}>
              ...
              <Text style={styles.paymentBadgeText}>{trip.payment_method}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  loadingText: { color: 'rgba(255,255,255,0.7)', marginTop: Spacing.sm, fontSize: Typography.sm },
  notFoundText: { color: 'rgba(255,255,255,0.7)', fontSize: Typography.lg, fontWeight: '600', marginTop: Spacing.md },
  backBtnCenter: {
    marginTop: Spacing.md, paddingHorizontal: Spacing.xl, paddingVertical: 12,
    backgroundColor: Colors.primary, borderRadius: BorderRadius.md,
  },
  backBtnCenterText: { color: '#fff', fontWeight: '700', fontSize: Typography.base },
  ...
  cancelNote: {
    ...
    backgroundColor: Colors.error + '10', borderRadius: 8, padding: 8, marginTop: Spacing.sm,
  },
  cancelNoteText: { flex: 1, fontSize: Typography.xs, color: Colors.error, textAlign: 'right' },
  driverRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: Spacing.sm, marginBottom: Spacing.md },
  ...
  priceDivider: { height: 1.5, backgroundColor: Colors.border, marginVertical: 4 },
  priceTotalRow: {},
