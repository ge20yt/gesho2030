...
export default function DriverProfileScreen() {
  ...
  // Real driver from Supabase, fallback to mock for display
  const mockDriver = MOCK_DRIVERS.find(d => d.id === id) ?? MOCK_DRIVERS[0];
  const [supabaseDriver, setSupabaseDriver] = useState<any>(null);
  const [driverLoading, setDriverLoading] = useState(true);

  // Derive display data from supabase or mock
  const driver = supabaseDriver ? {
    ...mockDriver,
    id: supabaseDriver.id,
    name: supabaseDriver.name,
    vehicle: supabaseDriver.vehicle,
    plate: supabaseDriver.plate,
    rating: Number(supabaseDriver.rating) || mockDriver.rating,
    reviewCount: supabaseDriver.total_trips || mockDriver.reviewCount,
    avatar: supabaseDriver.avatar_url || mockDriver.avatar,
    isOnline: supabaseDriver.is_online,
    ...
    const fetchDriver = async () => {
      try {
        ...
        const { data } = await supabase
          ...
          .eq('id', id)
          .single();
        if (data) setSupabaseDriver(data);
  ...
  const saveTrip = async (): Promise<string | null> => {
    ...
    try {
      ...
      // Use real supabase driver id if available, else try to get any available driver
      let realDriverId: string | null = supabaseDriver?.id ?? null;
      if (!realDriverId) {
        const { data: driverRow } = await supabase
          ...
          .limit(1)
          .single();
        realDriverId = driverRow?.id ?? null;
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          from_location: fromLocation || 'موقعك الحالي',
          to_location: toLocation || 'الوجهة المطلوبة',
          distance: '0 كم',
          duration: '0 دقيقة',
          price: finalPrice,
          status: 'pending',
