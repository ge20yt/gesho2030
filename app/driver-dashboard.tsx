...
import {
  ...
  ActivityIndicator, RefreshControl, Switch, Platform, Modal, Vibration,
} from 'react-native';
...
// ── New Trip Alert Modal ─────────────────────────────────────────
function NewTripAlertModal({
  visible,
  trip,
  onAccept,
  onReject,
  ...
  return (
    <Modal transparent animationType="slide" visible={visible} statusBarTranslucent>
      <View style={newTripStyles.overlay}>
        <View style={newTripStyles.backdrop} />
        <View style={newTripStyles.sheet}>
          {/* Pulse indicator */}
          <View style={newTripStyles.pulseWrap}>
            <View style={newTripStyles.pulseRing} />
            <View style={[newTripStyles.pulseRing, newTripStyles.pulseRing2]} />
            <View style={newTripStyles.pulseIcon}>
              <MaterialIcons name="directions-car" size={26} color="#fff" />
            </View>
          </View>

          <Text style={newTripStyles.title}>طلب رحلة جديدة!</Text>
          <Text style={newTripStyles.subtitle}>راكب قريب منك يحتاج سائق</Text>

          <View style={newTripStyles.tripCard}>
            <View style={newTripStyles.routeRow}>
              <MaterialIcons name="location-on" size={16} color={Colors.success} />
              <Text style={newTripStyles.routeText} numberOfLines={2}>{trip.from_location}</Text>
            </View>
            <View style={newTripStyles.routeConnector}>
              <View style={newTripStyles.connLine} />
            </View>
            <View style={newTripStyles.routeRow}>
              <MaterialIcons name="flag" size={16} color={Colors.error} />
              <Text style={newTripStyles.routeText} numberOfLines={2}>{trip.to_location}</Text>
            </View>
            <View style={newTripStyles.tripMeta}>
              <View style={newTripStyles.metaItem}>
                <MaterialIcons name="payments" size={14} color={Colors.primary} />
                <Text style={newTripStyles.metaText}>{Number(trip.price).toFixed(0)} ج.م</Text>
              </View>
              <View style={newTripStyles.metaItem}>
                <MaterialIcons name="straighten" size={14} color={Colors.textSecondary} />
                <Text style={newTripStyles.metaText}>{trip.distance}</Text>
              </View>
              <View style={newTripStyles.metaItem}>
                <MaterialIcons name="schedule" size={14} color={Colors.textSecondary} />
                <Text style={newTripStyles.metaText}>{trip.duration}</Text>
              </View>
            </View>
          </View>

          <View style={newTripStyles.btnRow}>
            <TouchableOpacity style={newTripStyles.rejectBtn} onPress={onReject} activeOpacity={0.85}>
              <MaterialIcons name="close" size={18} color={Colors.error} />
              <Text style={newTripStyles.rejectText}>رفض</Text>
            </TouchableOpacity>
            <TouchableOpacity style={newTripStyles.acceptBtn} onPress={onAccept} activeOpacity={0.85}>
              <MaterialIcons name="check" size={18} color="#fff" />
              <Text style={newTripStyles.acceptText}>قبول الرحلة</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
...
}: { visible: boolean; onClose: () => void; onSubmit: (stars: number) => void }) {
          ...
          {done ? (
            ...
            <>
              ...
              <View style={riderRatingStyles.starsRow}>
                {[1, 2, 3, 4, 5].map(i => (
                  <TouchableOpacity key={i} onPress={() => setStars(i)} hitSlop={{ top: 8, bottom: 8, left: 6, right: 6 }} activeOpacity={0.8}>
              ...
              <View style={riderRatingStyles.btnRow}>
                ...
                >
                  {submitting ? <ActivityIndicator color="#fff" size="small" /> : <Text style={riderRatingStyles.submitText}>إرسال</Text>}
                </TouchableOpacity>
              </View>
            </>
          )}
...
export default function DriverDashboardScreen() {
  ...
  // ── New trip auto-detection ─────────────────────────────────────
  const [newTripAlert, setNewTripAlert] = useState<TripRow | null>(null);
  const knownTripIdsRef = useRef<Set<string>>(new Set());
  const newTripPollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      isOnlineRef.current = driverData.is_online;
      ...
      if (tripsData) {
        setTrips(tripsData);
        tripsData.forEach((t: TripRow) => knownTripIdsRef.current.add(t.id));
      }
    } catch (e) {
  ...
  // ── Poll for new pending trips every 5 seconds ──────────────────
  const pollForNewTrips = useCallback(async () => {
    if (!driverIdRef.current || !isOnlineRef.current) return;
    try {
      const { data } = await supabase
        .from('trips')
        .select('id, user_id, from_location, to_location, price, distance, duration, status, created_at, payment_method')
        .eq('status', 'pending')
        .is('driver_id', null)
        .order('created_at', { ascending: false })
        .limit(5);
      ...
      const newTrip = data.find((t: TripRow) => !knownTripIdsRef.current.has(t.id));
      if (newTrip) {
        knownTripIdsRef.current.add(newTrip.id);
        // Vibrate on new trip
        try { Vibration.vibrate([0, 300, 100, 300]); } catch {}
        setNewTripAlert(newTrip as TripRow);
      }
    } catch { /* silent */ }
  }, []);

  useEffect(() => {
    newTripPollRef.current = setInterval(pollForNewTrips, 5000);
    return () => { if (newTripPollRef.current) clearInterval(newTripPollRef.current); };
  }, [pollForNewTrips]);

  // ── Accept new trip ─────────────────────────────────────────────
  const handleAcceptNewTrip = async () => {
    if (!newTripAlert || !driverIdRef.current) return;
    const tripToAccept = newTripAlert;
    setNewTripAlert(null);
    try {
      // Atomic: only update if driver_id is still null (prevent race condition)
      const { error } = await supabase
        .from('trips')
        .update({ driver_id: driverIdRef.current, status: 'active', updated_at: new Date().toISOString() })
        .eq('id', tripToAccept.id)
        .is('driver_id', null);

      if (error) {
        showAlert('تنبيه', 'تم قبول هذه الرحلة من سائق آخر بالفعل');
        return;
      }

      setTrips(prev => [{ ...tripToAccept, status: 'active', driver_id: driverIdRef.current! } as any, ...prev]);
      setActiveTab('active');
      showAlert('تم القبول!', 'تم قبول الرحلة بنجاح. توجه إلى الراكب الآن.');
    } catch { /* silent */ }
  };

  // ── Reject new trip ────────────────────────────────────────────
  const handleRejectNewTrip = () => {
    setNewTripAlert(null);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      locationWatchRef.current = await Location.watchPositionAsync(
        ...
        (newLoc) => { if (mounted) setDriverLocation({ latitude: newLoc.coords.latitude, longitude: newLoc.coords.longitude }); }
      );
    })();
    return () => { mounted = false; locationWatchRef.current?.remove(); };
  }, []);
  ...
  const toggleOnline = async () => {
    ...
    if (error) { showAlert('خطأ', 'فشل تحديث الحالة'); }
      ...
      setDriver(prev => prev ? { ...prev, is_online: newStatus } : prev);
    }
  ...
  const handleRateRider = async (tripId: string, riderId: string, stars: number) => {
    try {
      if (!driver?.id) return;
      await supabase.from('rider_ratings').insert({ trip_id: tripId, driver_id: driver.id, rider_id: riderId, rating: stars });
      setRatingRiderId(null);
    } catch { /* silent */ }
  };

  const handleArrived = async (tripId: string) => {
    await supabase.from('trips').update({ status: 'arrived', updated_at: new Date().toISOString() }).eq('id', tripId);
    setTrips(prev => prev.map(t => t.id === tripId ? { ...t, status: 'arrived' } : t));
  };

  const handleCompleteTrip = async (trip: TripRow) => {
    showAlert('تأكيد إتمام الرحلة', `هل تريد تأكيد إتمام الرحلة؟`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            const { error } = await supabase.from('trips').update({ status: 'completed', updated_at: new Date().toISOString() }).eq('id', trip.id);
            if (error) { showAlert('خطأ', 'فشل تحديث حالة الرحلة'); return; }
            if (driver) {
              await supabase.from('drivers').update({ total_trips: (driver.total_trips ?? 0) + 1, updated_at: new Date().toISOString() }).eq('id', driver.id);
              setDriver(prev => prev ? { ...prev, total_trips: (prev.total_trips ?? 0) + 1 } : prev);
            }
            if (trip.user_id) await awardTripCompletionPoints(trip.user_id, Number(trip.price), trip.from_location, trip.to_location);
            setTrips(prev => prev.map(t => t.id === trip.id ? { ...t, status: 'completed' } : t));
            showAlert('تم بنجاح', 'تم إتمام الرحلة ومنح النقاط للراكب');
          } finally { setCompletingTrip(null); }
        },
      },
    ]);
  };
  ...
  const handleCancelTrip = async (tripId: string) => {
    showAlert('إلغاء الرحلة', 'هل أنت متأكد من إلغاء هذه الرحلة؟', [
      ...
      {
        text: 'إلغاء الرحلة', style: 'destructive',
        onPress: async () => {
  ...
  const handleStartTrip = async (tripId: string) => {
    await supabase.from('trips').update({ status: 'active', updated_at: new Date().toISOString() }).eq('id', tripId);
  ...
  const dailyEarnings = trips.filter(t => t.status === 'completed' && new Date(t.created_at).toDateString() === today).reduce((sum, t) => sum + Number(t.price), 0);
  const totalEarnings = trips.filter(t => t.status === 'completed').reduce((sum, t) => sum + Number(t.price), 0);
  const completedToday = trips.filter(t => t.status === 'completed' && new Date(t.created_at).toDateString() === today).length;
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {/* New Trip Alert Modal */}
      <NewTripAlertModal
        visible={newTripAlert !== null}
        trip={newTripAlert}
        onAccept={handleAcceptNewTrip}
        onReject={handleRejectNewTrip}
      />
        ...
        <View style={styles.driverCard}>
          <View style={styles.driverCardContent}>
            ...
            <Image
              ...
              style={styles.driverAvatar} contentFit="cover" transition={200}
            />
          </View>
          <View style={[styles.statusBar, { backgroundColor: driver.is_online ? Colors.success + '20' : Colors.error + '15' }]}>
            ...
            <Text style={[styles.statusText, { color: driver.is_online ? Colors.success : Colors.offline }]}>
              {driver.is_online ? 'أنت متاح الآن — ستظهر للركاب القريبين' : 'أنت غير متاح — فعّل الاستقبال لقبول الرحلات'}
            </Text>
          </View>
        </View>
      </LinearGradient>
      ...
      >
        ...
        <View style={styles.mapSection}>
          <TouchableOpacity style={styles.mapToggleBtn} onPress={() => setShowMap(v => !v)} activeOpacity={0.85}>
            ...
            <Text style={styles.mapToggleText}>{showMap ? 'إخفاء الخريطة الحية' : 'عرض الخريطة الحية'}</Text>
            ...
            <View style={styles.mapContainer}>
              {Platform.OS !== 'web' && MapView && driverLocation ? (
                <MapView
                  ...
                  region={{ latitude: driverLocation.latitude, longitude: driverLocation.longitude, latitudeDelta: 0.015, longitudeDelta: 0.015 }}
                ...
                <View style={styles.mapFallback}>
                  ...
                  <Text style={styles.mapFallbackText}>
                    {driverLocation ? `موقعك: ${driverLocation.latitude.toFixed(5)}, ${driverLocation.longitude.toFixed(5)}` : 'جارٍ تحديد موقعك...'}
                  </Text>
              ...
              {driverLocation && (
                <View style={styles.coordBadge}>
                  ...
                  <Text style={styles.coordText}>{driverLocation.latitude.toFixed(5)}, {driverLocation.longitude.toFixed(5)}</Text>
            ...
            {([
              ...
              >
                ...
                <Text style={[styles.tabText, activeTab === tab.key && styles.tabTextActive]}>
                  {tab.label} ({trips.filter(t => tab.key === 'active' ? (t.status === 'active' || t.status === 'arrived') : t.status === tab.key).length})
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {filteredTrips.length === 0 ? (
            <View style={styles.emptyTrips}>
              ...
              <Text style={styles.emptyText}>
                {activeTab === 'pending' ? 'لا توجد رحلات معلقة' : activeTab === 'active' ? 'لا توجد رحلات جارية' : 'لا توجد رحلات مكتملة'}
              </Text>
            </View>
          ) : (
            filteredTrips.map(trip => (
              <TripCard
                ...
                onRateRider={trip.status === 'arrived' ? () => setRatingRiderId({ tripId: trip.id, riderId: trip.user_id }) : undefined}
              />
            ))
          )}
        </View>
...
function TripCard({ trip, isCompleting, onComplete, onStart, onArrived, onCancel, onRateRider }: {
  trip: TripRow; isCompleting: boolean;
  onComplete: () => void; onStart: () => void; onArrived: () => void; onCancel: () => void; onRateRider?: () => void;
}) {
    ...
    <View style={tripCardStyles.card}>
      ...
      {trip.status === 'pending' && (
        <View style={tripCardStyles.actions}>
          <TouchableOpacity style={[tripCardStyles.actionBtn, { backgroundColor: Colors.error + '15', borderColor: Colors.error + '40' }]} onPress={onCancel}>
            <MaterialIcons name="close" size={14} color={Colors.error} />
            <Text style={[tripCardStyles.actionText, { color: Colors.error }]}>إلغاء</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[tripCardStyles.actionBtn, { backgroundColor: Colors.primary + '15', borderColor: Colors.primary + '40' }]} onPress={onStart}>
      ...
      {trip.status === 'active' && (
        <View style={tripCardStyles.actions}>
          <TouchableOpacity style={[tripCardStyles.actionBtn, { backgroundColor: Colors.accent + '20', borderColor: Colors.accent + '50', flex: 1 }]} onPress={onArrived}>
      ...
      {trip.status === 'arrived' && (
        <View style={tripCardStyles.actions}>
          {onRateRider && (
            <TouchableOpacity style={[tripCardStyles.actionBtn, { backgroundColor: Colors.accent + '15', borderColor: Colors.accent + '40' }]} onPress={onRateRider}>
          ...
          <TouchableOpacity style={[tripCardStyles.actionBtn, tripCardStyles.completeBtn, isCompleting && { opacity: 0.6 }]} onPress={onComplete} disabled={isCompleting}>
...
const styles = StyleSheet.create({
  ...
  notifBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.1)', alignItems: 'center', justifyContent: 'center' },
  ...
  driverCard: {
    ...
    borderRadius: BorderRadius.xl, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)',
  },
  ...
  statusBar: { flexDirection: 'row-reverse', alignItems: 'center', gap: 6, padding: Spacing.sm, paddingHorizontal: Spacing.md },
  ...
  mapSection: {
    ...
    borderRadius: BorderRadius.xl, overflow: 'hidden', borderWidth: 1, borderColor: Colors.borderLight, ...Shadows.sm,
  },
  mapToggleBtn: { flexDirection: 'row-reverse', alignItems: 'center', gap: 8, paddingHorizontal: Spacing.md, paddingVertical: 14 },
  ...
  mapFallback: { height: 160, alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: Colors.bgLight },
  ...
  coordBadge: {
    ...
    backgroundColor: 'rgba(0,0,0,0.7)', borderRadius: BorderRadius.full, paddingHorizontal: 10, paddingVertical: 5,
  },
  ...
  statsGrid: { flexDirection: 'row-reverse', flexWrap: 'wrap', gap: Spacing.sm, padding: Spacing.md, paddingBottom: 0 },
  actionsRow: { flexDirection: 'row-reverse', gap: Spacing.sm, paddingHorizontal: Spacing.md, paddingVertical: Spacing.md },
  actionBtn: {
    flex: 1, alignItems: 'center', gap: 4, paddingVertical: Spacing.sm,
    backgroundColor: Colors.bgWhite, borderRadius: BorderRadius.lg, borderWidth: 1, borderColor: Colors.borderLight, ...Shadows.sm,
  },
  ...
  tripsSection: {
    ...
    borderRadius: BorderRadius.xl, padding: Spacing.md, borderWidth: 1, borderColor: Colors.borderLight, ...Shadows.sm,
  },
  ...
  tab: {
    ...
    paddingVertical: 8, borderRadius: BorderRadius.md, backgroundColor: Colors.bgLight, borderWidth: 1, borderColor: Colors.borderLight,
  },
  ...
  registerBtn: { marginTop: Spacing.lg, backgroundColor: Colors.primary, borderRadius: BorderRadius.md, paddingHorizontal: 32, paddingVertical: 14 },
  registerBtnText: { color: '#fff', fontWeight: '700', fontSize: Typography.base },
});

const statStyles = StyleSheet.create({
  card: { width: '48%', backgroundColor: Colors.bgWhite, borderRadius: BorderRadius.lg, padding: Spacing.md, alignItems: 'center', gap: 4, borderWidth: 1, ...Shadows.sm },
...
const tripCardStyles = StyleSheet.create({
  card: { backgroundColor: Colors.bgLight, borderRadius: BorderRadius.lg, padding: Spacing.md, marginBottom: Spacing.sm, borderWidth: 1, borderColor: Colors.borderLight },
  ...
  actionBtn: { flex: 1, flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 9, borderRadius: BorderRadius.md, borderWidth: 1 },
  completeBtn: { backgroundColor: Colors.success, borderColor: Colors.success },
  actionText: { fontSize: Typography.xs, fontWeight: '700' },
});

const newTripStyles = StyleSheet.create({
  ...
  sheet: {
    ...
    shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.2, shadowRadius: 16, elevation: 12,
  },
  pulseWrap: { position: 'relative', width: 72, height: 72, alignItems: 'center', justifyContent: 'center', marginBottom: Spacing.md },
  pulseRing: {
    position: 'absolute', width: 72, height: 72, borderRadius: 36,
    backgroundColor: Colors.success + '25', borderWidth: 2, borderColor: Colors.success + '40',
  },
  pulseRing2: { width: 56, height: 56, borderRadius: 28, backgroundColor: Colors.success + '35' },
  pulseIcon: {
    width: 52, height: 52, borderRadius: 26, backgroundColor: Colors.success,
    alignItems: 'center', justifyContent: 'center',
    shadowColor: Colors.success, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 8, elevation: 6,
  },
  title: { fontSize: Typography.xxl, fontWeight: '900', color: Colors.textPrimary, marginBottom: 4 },
  subtitle: { fontSize: Typography.sm, color: Colors.textSecondary, marginBottom: Spacing.lg },
  tripCard: {
    width: '100%', backgroundColor: Colors.bgLight, borderRadius: BorderRadius.xl,
    padding: Spacing.md, marginBottom: Spacing.lg, borderWidth: 1, borderColor: Colors.borderLight,
  },
  routeRow: { flexDirection: 'row-reverse', alignItems: 'flex-start', gap: 8, paddingVertical: 6 },
  routeText: { flex: 1, fontSize: Typography.sm, color: Colors.textPrimary, fontWeight: '500', textAlign: 'right', lineHeight: 20 },
  routeConnector: { paddingRight: 7 },
  connLine: { width: 2, height: 16, backgroundColor: Colors.border, marginVertical: -4 },
  tripMeta: { flexDirection: 'row-reverse', flexWrap: 'wrap', gap: Spacing.sm, paddingTop: Spacing.sm, borderTopWidth: 1, borderTopColor: Colors.borderLight, marginTop: 4 },
  metaItem: { flexDirection: 'row-reverse', alignItems: 'center', gap: 4 },
  metaText: { fontSize: Typography.xs, color: Colors.textSecondary, fontWeight: '600' },
  ...
  rejectBtn: {
    ...
    paddingVertical: 14, borderRadius: BorderRadius.md, borderWidth: 1.5, borderColor: Colors.error + '60',
    backgroundColor: Colors.error + '10',
  },
  rejectText: { fontSize: Typography.base, fontWeight: '700', color: Colors.error },
  acceptBtn: {
    flex: 2, flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', gap: 6,
    paddingVertical: 14, borderRadius: BorderRadius.md, backgroundColor: Colors.success,
    shadowColor: Colors.success, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 8, elevation: 6,
  },
  acceptText: { fontSize: Typography.base, fontWeight: '700', color: '#fff' },
});

const riderRatingStyles = StyleSheet.create({
  overlay: { flex: 1, justifyContent: 'flex-end' },
  backdrop: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.55)' },
  sheet: { backgroundColor: Colors.bgWhite, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: Spacing.xl, paddingBottom: Spacing.xl + 8, alignItems: 'center', ...Shadows.lg },
  ...
  skipBtn: { flex: 1, paddingVertical: 14, borderRadius: BorderRadius.md, borderWidth: 1.5, borderColor: Colors.border, alignItems: 'center' },
  skipText: { fontSize: Typography.base, color: Colors.textSecondary, fontWeight: '600' },
  submitBtn: { flex: 2, paddingVertical: 14, borderRadius: BorderRadius.md, backgroundColor: Colors.primary, alignItems: 'center' },
  submitDisabled: { opacity: 0.5 },
  submitText: { fontSize: Typography.base, color: '#fff', fontWeight: '700' },
  success: { alignItems: 'center', paddingVertical: Spacing.xl },
  successIcon: { width: 60, height: 60, borderRadius: 30, backgroundColor: Colors.success, alignItems: 'center', justifyContent: 'center', marginBottom: Spacing.md },
